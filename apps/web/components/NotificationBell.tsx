          ...
          <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 z-50 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80">
              ...
              <div className="flex items-center gap-1">
                {count > 0 && onDismissAll && (
                  <button
                    type="button"
                    onClick={() => { onDismissAll(); setOpen(false); }}
                    className="text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 px-2 py-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                  >
                    Clear all
                  </button>
                )}
                    ...
                    <div key={n.id} className={`px-4 py-3.5 flex items-start gap-3 ${meta.bg}`}>
                      ...
                      {onDismiss && (
                        <button
                          ...
                          className="shrink-0 mt-0.5 text-slate-300 hover:text-slate-500 transition-colors p-0.5 rounded"
                          aria-label="Dismiss"
                        >
